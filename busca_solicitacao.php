...
WHERE 
    ...
    while ($registro = $resultadoBuscaDividas->fetch_object()) {
        ...
        if (strtotime($registro->vencimento) >= strtotime(date('Y-m-d'))) {
            if($registro->status == 'Negociado'){
                $array['vencimento'] = "<span class='badge badge-success' style='height: 19px; width: 90px; font-size: 87%';>$datetimeformat</span>";
            }else if ($diasVencidos <= 2) {
                $array['vencimento'] = "<span class='badge badge-warning' style='height: 19px; width: 90px; font-size: 87%';>$datetimeformat</span>";
            } else{
                $array['vencimento'] = "<span class='badge badge-info' style='height: 19px; width: 90px; font-size: 87%';>$datetimeformat</span>";
            }
            ...
            if($registro->status == 'Negociado'){
                ...
                $array['vencimento'] = "<span class='badge badge-danger' style='height: 19px; width: 90px; font-size: 87%';>$datetimeformat</span>";
            }
            ...
            if ($registro->cobranca == 'D') {
                ...
                $i = 30;
                $j = 0;
                while ($i <= $diasVencidos) {
                    $i += 30;
                    $j++;
                }
                $valorJuros = $registro->juros * $j++;
            } else if ($registro->cobranca == 'A') {
                $anos = $diasVencidos / 365;
                $valorJuros = $registro->juros * floor($anos);
            }
            ///////////////////////////////////////////////////////////
